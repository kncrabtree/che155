...
The area \\(A\\) can be measured from the NMR spectrum and then used to infer the relative concentration of a molecule. This can be done either by numerical integration or by curve fitting. An example NMR spectrum of a single peak is shown below:

<!-- TODO: example NMR spectrum here -->

A single peak in an NMR spectrum has a **Lorentzian** lineshape. In other words, the shape of the peak as a function of chemical shift is
...
- \\(w\\) is the **full width at half maximum** (FWHM) of the peak, where \\(f(\delta\pm w/2) = f(\delta_0)/2\\).
- \\(\delta_0\\) is the exact center of the peak, where \\(\delta = \delta_0\\).

<!-- TODO: example Lorentzian function, annotated -->
...
\\[ S = \sum_{i=1}^N r_i = \sum_{i=1}^N (y_i - f(x_i))^2 \\]
...
- [nmr-analysis.ipynb](https://github.com/kncrabtree/che155/blob/master/_notebooks/week3/nmr-analysis.ipynb) - [Open in Google Colab](https://colab.research.google.com/github/kncrabtree/che155/blob/master/_notebooks/week3/nmr-analysis.ipynb) - [Download](https://raw.githubusercontent.com/kncrabtree/che155/master/_notebooks/week3/nmr-analysis.ipynb)
